...
impl ElfReader {
    pub fn new(id: u32, file: &str) -> Self {
        ...
        let (sym_t, str_t) = file_stream
            .symbol_table()
            .expect("Section table should be parseable")
            .expect("File should have a .Symtab section");
        let mut func_vec = sym_t
            .iter()
            .filter(|x| x.st_symtype() == STT_FUNC)
            .enumerate()
            .map(|(idx, x)| {
                let func_name = str_t
                    .get(x.st_name as usize)
                    .expect("Invalid index of StringTable");
                let func_start = x.st_value;
                let func_end = x.st_size + func_start;

                let func_type = if func_start == func_end && func_start == 0 {
            ...
            .filter(|x| x.func_type == FunType::LocalFunc)
            .collect::<Vec<Func>>();
        ...
        func_vec.iter().for_each(|x| {
            ...
            debug_println!(
                "Get function: {}, id: {}, start: 0x{:X}, end: 0x{:X}, type: {:?}",
                x.name,
                x.id,
                x.start,
                x.end,
        ...
        let start = func_vec
            .first()
            .expect("Failed to get the first elements in func_vec")
            .start;
        let end = func_vec
            .last()
            .expect("Failed to get the last elements in func_vec")
            .end;
    ...
    pub fn find(&self, value: u64) -> Option<&Func> {
        self.func_vec
            ...
            .ok()
            .and_then(|idx| self.func_vec.get(idx))
    }

    pub fn get_func(&self, id: u32) -> Option<&Func> {
        self.func_vec.get(id as usize).and_then(|x| {
            if x.id == id {
                Some(x)
            } else {
...
mod tests {
    ...
    fn test_reader_new() {
        ...
        println!(
            "ElfReader(id: {}, name: {}, start: 0x{:X}, end: 0x{:X}, func_num: {})",
            elf_reader0.id,
            elf_reader0.name,
            elf_reader0.start,
            elf_reader0.end,
            elf_reader0.func_vec.len()
        );
    ...
    fn test_find() {
        ...
        for i in &elf_reader.func_vec {
            if i.start > last_end {
                ...
                print!(
                    "Random addr 0x{:X} between 0x{:X} and 0x{:X}, should miss\t",
                    rand_addr, i.start, last_end
                );
    ...
    fn test_find1() {
        ...
        for i in 0..elf_reader1.func_vec.len() {
            ...
            print!(
                "Random addr: 0x{:X}, start: 0x{:X}, end: 0x{:X}\t",
                rand_addr, start, end
            );
    ...
    fn test_get_func() {
        ...
        assert!(elf_reader
            .get_func(elf_reader.func_vec.len() as u32)
            .is_none());
    }
}
